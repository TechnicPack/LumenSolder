...
class ApiTest extends TestCase {
	...
	public function testBase()
	{
		...
		$this->assertEquals('{"api":"SolderAPI","version":"0.1","stream":"dev"}', $response->getContent());
	}
	...
	{
		$response = $this->call('GET', '/modpack/bob');
		$this->assertResponseOk();
		$this->assertTrue(is_a($response,'Illuminate\Http\JsonResponse'));
		$json = $response->getData(true);
	...
	public function testModpackSlug()
	{
		...
		$response = $this->call('GET', '/modpack/'.$modpack->slug);
		$this->assertResponseOk();
		$this->assertTrue(is_a($response,'Illuminate\Http\JsonResponse'));
		$json = $response->getData(true);
	...
	{
		$response = $this->call('GET', '/mod/bob');
		$this->assertResponseOk();
		$this->assertTrue(is_a($response,'Illuminate\Http\JsonResponse'));
		$json = $response->getData(true);
	...
	{
		$mod = Mod::find(1);
		$response = $this->call('GET', '/mod/'.$mod->name);
		$this->assertResponseOk();
		$this->assertTrue(is_a($response,'Illuminate\Http\JsonResponse'));
		$json = $response->getData(true);
