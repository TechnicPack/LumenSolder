...
class APIController extends BaseController {
	...
	public function getModpack(Request $request, $modpack = null, $build = null)
	{
		if (empty($modpack))
		{
			...
			if (empty($build))
				...
				return response()->json($this->fetchBuild($request, $modpack, $build));
		}
	}
	...
	private function fetchModpacks()
	{
		...
		$response['mirror_url'] = env('SOLDER_MIRROR_URL');
	...
	private function fetchBuild(Request $request, $slug, $build)
	{
			...
			if (Cache::has('modpack.'.$slug.'.build.'.$buildpass.'modversion') && empty($this->client) && empty($this->key))
			{
				...
				foreach ($build->modversions as $modversion)
				{
					$response['mods'][] = array(
												...
												"md5" => $modversion->md5,
												"url" => env('SOLDER_MIRROR_URL').'mods/'.$modversion->mod->name.'/'.$modversion->mod->name.'-'.$modversion->version.'.zip'
												);
				}
		...
		} else if ($request->input('include') == "mods") {
